## Norushen

### Overview

- 25-man setup:
  - Normal: 2 tanks, 5-6 healers.
  - Heroic: 2 tanks, 4-5 healers, high single-target DPS.
- 7 minute enrage.
- You fight the `Amalgam of Corruption`.
- Corruption reduces boss damage only; it does not reduce damage to adds.
- Normal starts players at `75` corruption.
- Heroic starts players at `50` corruption.

### Positioning

- Stack near the boss for healing and add control.
- Tanks keep boss stable and swap for the stacking physical tank hit.
- When a `Manifestation of Corruption` dies, move it a few yards out before death so the pool does not drop under the raid.
- Dodge the rotating beam.
  - If it spawns on the stack, sidestep first.
  - If it is coming toward you, run through/around the boss to the safe side.

### Corruption Plan

- Normal goal:
  - Purify as many players as possible before boss reaches `50%`.
  - Stop sending players down at `50%`.
  - Burn boss and handle add spawns from `50%` to dead.
- 25-man orb set:
  - Send 2 DPS, 1 tank, and 1 healer per set.
  - Use a pre-assigned rotation.
  - DPS going in soon should soak nearby pools before entering if needed.
- Consider holding boss around `60%` if too many players still need purification.
- Leave 2-3 minutes for the final `50%` burn.

### Trial Realms

#### Tank realm

- Survive 1 minute against `Titanic Corruption`.
- Avoid `Titanic Smash` frontal.
- Interrupt `Hurl Corruption`.
- Use mitigation for:
  - `Burst of Corruption`: heavy Shadow damage.
  - `Piercing Corruption`: heavy Physical damage.
- Returns tank to 0 corruption.

#### Healer realm

- Kill `Greater Corruption` to exit.
- Heal NPCs to increase their damage.
- Damage the add when possible.
- Dispel the healer DoT quickly.
- Returns healer to 0 corruption and increases healing done.

#### DPS realm

- Kill every add within 1 minute to exit purified.
- Kill `Manifestation of Corruption` first.
  - Avoid `Tear Reality` frontal.
- Kill `Essence of Corruption` after.
  - Attack from behind; front shield blocks damage.
  - Dodge `Expel Corruption` lines.
- Every DPS-realm add killed spawns a stronger version outside for the raid.
- Do not send too many DPS at once.

### Outside Adds

- `Essence of Corruption`
  - Stun/interrupt until dead.
  - If its line reaches the boss, boss gains a stacking damage buff.
- `Manifestation of Corruption`
  - Tank immediately.
  - Entire raid swaps and kills it fast.
  - Pulses raid damage while alive.
  - Drops a shadow pool on death.
- Shadow pools:
  - Massive raid damage while active.
  - Must be soaked within a couple seconds.
  - Soaker gains 25 corruption.
  - Players at 100 corruption cannot soak.
  - Prefer assigned players who are about to purify, but anyone should soak rather than leave it up.
- In 25-man, healers are strong pool soakers because they can re-enter and purify again.

### Final Burn

- At `50%`, boss spawns a `Manifestation of Corruption` every 10% health.
- Stop sending players into trials.
- Stack, burn boss, and kill each large add immediately.
- `Icy Fear` raid damage increases as boss health gets lower.
- Chain raid cooldowns through the final burn.
- Soak every pool instantly.

#### Heroic notes

- Heroic is primarily a DPS check.
- Stack in melee and Bloodlust on pull.
- Do not send DPS into trials unless one player is far ahead and the raid needs that net gain.
- Send healers in 1-2 at a time to purify for increased healing.
- `Manifestation of Corruption` pulses give 2 corruption to the raid in 25-man.
- Pools also pulse 2 corruption in 25-man.
- From `50%`, add handling must be immediate.
  - If a large add pulses more than once, the pull is likely behind the enrage timer.
  - Assign a healer to stand ready at each add death and soak the pool instantly.

### Assignments

- Tanks:
  - Swap the boss to reset stacking tank damage.
  - Normal: rotate tanks through trials.
  - Pick up outside `Manifestations` immediately.
- Healers:
  - Normal: rotate through healer trials and help soak pools when assigned.
  - Heroic: 1-2 healers at a time purify early; assign pool soakers after `50%`.
  - Plan CDs for final burn.
- DPS:
  - Normal: 2 DPS per orb set.
  - Call when a DPS-realm add dies so outside raid can swap.
  - Heroic: stay out and maximize boss damage unless specifically assigned.

### Common Wipe Causes

- Sending too many DPS into trials and overwhelming outside adds.
- `Essence` casts reaching the boss.
- `Manifestation` pools not soaked instantly.
- Boss pushed below `50%` before enough players are purified on normal.
- Heroic: unnecessary DPS trials creating too much corruption and losing boss uptime.
- Dying to rotating beam during burn.

### Quick Reminders

- Normal: 2 DPS, 1 tank, 1 healer per orb set.
- Purify before `50%`; stop trials at `50%`.
- Swap to big adds immediately.
- Move big adds out before death; soak pools fast.
- Heroic: stack, lust pull, send healers only, delete adds after `50%`.

### Video Guides

- Normal video guide (YouTube): https://youtu.be/7oFoDLYxQag
- Heroic video guide (YouTube): https://youtu.be/U5t6-0-f-Xk
- Heroic written guide (Icy Veins): https://www.icy-veins.com/wow/norushen-strategy-guide-normal-heroic
